#include <defs.h>
#ifndef _LA64_TRAPFRAME_H_
#define _LA64_TRAPFRAME_H_
/* $2 - $31 */
struct pushregs {
    uint64_t reg_r[30];         //modified
};
...
struct trapframe {
    uint64_t tf_badv; // CSR[0x7]   modified
    uint32_t tf_estat; // CSR[0x5]
    uint32_t tf_prmd; // CSR[0x1]
    uint64_t tf_ra;
    struct pushregs tf_regs;
    uint64_t tf_era; // CSR[0x6] same as epc on LOONGARCH
};

/*
 * LA64 exception codes.
 */
