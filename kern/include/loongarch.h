...
static inline uint8_t inb(uint64_t port) __attribute__((always_inline));
static inline void outb(uint64_t port, uint8_t data) __attribute__((always_inline));
static inline uint64_t inw(uint64_t port) __attribute__((always_inline));
static inline void outw(uint64_t port, uint64_t data) __attribute__((always_inline));
...
static inline void
outb(uint64_t port, uint8_t data) {
    *((volatile uint8_t*) port) = data;
}

static inline uint64_t
inw(uint64_t port) {
    uint64_t data = *((volatile uint64_t *) port);
    return data;
}

static inline void
outw(uint64_t port, uint64_t data) {
    *((volatile uint64_t *) port) = data;
}


/* board specification */
#define COM1            0x800000001fe001e0
#define COM1_IRQ        1
