...
SLASH	:= /
V       := @
GCCPREFIX:= loongarch64-linux-gnu-
QEMU:= qemu-system-loongarch64
QEMUOPTS:= -M virt -m 1G -nographic
# use the same qemu as use_for_linux
TERMINAL := gnome-terminal
TERMINALOPT := -e
# eliminate default suffix rules  添加后缀规则可用的后缀
.SUFFIXES: .c .S .h
...
# -fno-builtin-fprintf:  Use all built-in functions except fprintf()
# -fno-builtin:  Avoid reporting errors with the same name as built-in functions/关闭内置函数优化选项
# -nostdlib:  System standard boot files and standard library files are not connected, only specified files are passed to the connector.
# -nostdinc:  Make the compiler no longer look for header files in the system's default header directory
# -g:  Generate debug information
# -G0:  ?
# -Wa:  此选项传递 option 给汇编程序; 如果 option 中间有逗号, 就将 option 分成多个选项,然后传递给会汇编程序
# -O0:  编译器的优化选项的级别:没有优化
# -msoft-float:  直接生成浮点运算的指令
# -ggdb:  此选项将尽可能的生成 gdb 的可以使用的调试信息
# -gstabs:  此选项以 stabs 格式声称调试信息, 但是不包括 gdb 调试信息
CFLAGS	:= $(LAB_FLA) -fno-builtin-fprintf -fno-builtin -nostdlib  -nostdinc -g -G0 -fno-pic -msoft-float -ggdb
LD      := $(GCCPREFIX)ld
AR      := $(GCCPREFIX)ar
LDFLAGS	+= -nostdlib -m elf64loongarch
...
# search directory: kern/init kern/driver ...
SRC_DIR   := $(addprefix $(SRCDIR)/,$(MODULES))
BUILD_DIR := $(addprefix $(OBJDIR)/,$(MODULES))
DEP_DIR   := $(addprefix $(DEPDIR)/,$(MODULES))
# define the system variable-file search path
VPATH     += $(SRC_DIR)
# Expand the.c file in the directory
SRC       := $(foreach sdir,$(SRC_DIR),$(wildcard $(sdir)/*.c))
# Generate target file name of the .c file
OBJ       := $(patsubst $(SRCDIR)/%.c, $(OBJDIR)/%.o, $(SRC))
# Expand the.S file in the directory
ASMSRC    := $(foreach sdir,$(SRC_DIR),$(wildcard $(sdir)/*.S))
# Generate target file name of the .S file
OBJ       += $(patsubst $(SRCDIR)/%.S, $(OBJDIR)/%.o, $(ASMSRC))
# Defines the directory to look for header files
...
#Generate depend file name of the .c file
...
gdb: $(OBJDIR)/ucore-kernel-$(USER_OBJ_MODE)
	$(V)$(GDB) $(OBJDIR)/ucore-kernel-$(USER_OBJ_MODE) -x tools/gdbinit
...
$1: $(BUILD_DIR) $(addsuffix .o,$1) $(USER_LIB)
	$(V)$(LD) -nostdlib -m elf64loongarch -T $(USER_LIB_SRCDIR)/user.ld  $(addsuffix .o,$1) --whole-archive $(USER_LIB) -o $$@
